...
for n in range(len(phases)):
    if glaciers=='Y':
        ...
        os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Final_products/'+str(watershed)+'/'+str(year)+'/Maps/SnowDepth/resolution_'+str(resolution1)+'m/')
        cmd='gdalwarp -overwrite -tr 2.0 2.0 -r average -dstnodata "-9999" -of GTiff '+str(extent)+'_'+str(year)+'_'+str(phases[n])+'_SnowDepth_lakemodel'+str(lakemodel)+'_'+str(resolution1)+'m.tif '+str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Final_products/'+str(watershed)+'/'+str(year)+'/Maps/SnowDepth/resolution_'+str(resolution2)+'m/'+str(extent)+'_'+str(year)+'_'+str(phases[n])+'_SnowDepth_lakemodel'+str(lakemodel)+'_'+str(resolution2)+'m.tif'
        subprocess.run([x for x in cmd.split(" ") if x != ""])
        os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Final_products/'+str(watershed)+'/'+str(year)+'/Maps/SnowDepth/resolution_'+str(resolution2)+'m/')
        [R,SD]=np.array(pyrsgis.raster.read(str(extent)+'_'+str(year)+'_'+str(phases[n])+'_SnowDepth_lakemodel'+str(lakemodel)+'_'+str(resolution2)+'m.tif', bands='all'))
        pyrsgis.raster.export(SD, R, filename=str(extent)+'_'+str(year)+'_'+str(phases[n])+'_SnowDepth_lakemodel'+str(lakemodel)+'_'+str(resolution2)+'m.tif')
...
# Downsample DEM model input data to 10 m
inputs = ['_Slope','_Eastness','_Northness','','_Curvature','_Aspect'] # Input model data. The empy '' is for the bare earth file
for m in range(len(inputs)):
    os.chdir(str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/DEM/v'+str(BEversion)+'/resolution_'+str(resolution1)+'m/')
    cmd='gdalwarp -overwrite -tr 10.0 10.0 -r average -dstnodata "-9999" -of GTiff '+str(extent)+str(inputs[m])+'_BE_v'+str(BEversion)+'_'+str(resolution1)+'m.tif '+str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/DEM/v'+str(BEversion)+'/resolution_'+str(resolution3)+'m/'+str(extent)+str(inputs[m])+'_BE_v'+str(BEversion)+'_'+str(resolution3)+'m.tif'
    subprocess.run([x for x in cmd.split(" ") if x != ""])
    os.chdir(str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/DEM/v'+str(BEversion)+'/resolution_'+str(resolution3)+'m/')
    [R,y]=np.array(pyrsgis.raster.read(str(extent)+str(inputs[m])+'_BE_v'+str(BEversion)+'_'+str(resolution3)+'m.tif', bands='all'))
    pyrsgis.raster.export(y, R, filename=str(extent)+str(inputs[m])+'_BE_v'+str(BEversion)+'_'+str(resolution3)+'m.tif')
...
# Downsample canopy model input data to 10 m
inputs = ['_CC','_CH', '_CD'] # Input model data.
for m in range(len(inputs)):
    os.chdir(str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/Canopy/v'+str(CANversion)+'/resolution_'+str(resolution1)+'m/')
    cmd='gdalwarp -overwrite -tr 10.0 10.0 -r average -dstnodata "-9999" -of GTiff '+str(extent)+str(inputs[m])+'_v'+str(CANversion)+'_'+str(resolution1)+'m.tif '+str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/Canopy/v'+str(CANversion)+'/resolution_'+str(resolution3)+'m/'+str(extent)+str(inputs[m])+'_v'+str(CANversion)+'_'+str(resolution3)+'m.tif'
    subprocess.run([x for x in cmd.split(" ") if x != ""])
    os.chdir(str(drive)+':/LiDAR_data_processing/Bare_earth/'+str(watershed)+'/Canopy/v'+str(CANversion)+'/resolution_'+str(resolution3)+'m/')
    [R,y]=np.array(pyrsgis.raster.read(str(extent)+str(inputs[m])+'_v'+str(CANversion)+'_'+str(resolution3)+'m.tif', bands='all'))
    pyrsgis.raster.export(y, R, filename=str(extent)+str(inputs[m])+'_v'+str(CANversion)+'_'+str(resolution3)+'m.tif')
...
# Downsample Watershed Mask to 10 m
lakes = '_no_lakes'
glaciers = ''
for m in range(len(subbasin)):
    os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Watershed_mask/resolution_'+str(resolution1)+'m/')
    cmd='gdalwarp -overwrite -tr 10.0 10.0 -r near -dstnodata "-9999" -of GTiff '+str(subbasin[m])+'_watershed'+str(lakes)+str(glaciers)+'_'+str(resolution1)+'m.tif '+str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Watershed_mask/resolution_'+str(resolution3)+'m/'+str(subbasin[m])+'_watershed'+str(lakes)+str(glaciers)+'_'+str(resolution3)+'m.tif'
    subprocess.run([x for x in cmd.split(" ") if x != ""])
    os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Watershed_mask/resolution_'+str(resolution3)+'m/')
    [R,y]=np.array(pyrsgis.raster.read(str(subbasin[m])+'_watershed'+str(lakes)+str(glaciers)+'_'+str(resolution3)+'m.tif', bands='all'))
    pyrsgis.raster.export(y, R, filename=str(subbasin[m])+'_watershed'+str(lakes)+str(glaciers)+'_'+str(resolution3)+'m.tif')
...
nodata = -9999
phase = 'D1'
filename = 'ENG_SS_260416_V2_1m_clip.tif'

# Get the desired extent. You need a shapefile (polygon) of the area for this.
WS = geopandas.read_file(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Watershed_mask/resolution_'+str(resolution1)+'m/vector/'+str(extent)+'_extent.shp')
ext = WS.bounds
bounds = (ext['minx'][0].round(0), ext['miny'][0].round(0), ext['maxx'][0].round(0), ext['maxy'][0].round(0))
rows=ext['maxy'][0].round(0) - ext['miny'][0].round(0)
cols=ext['maxx'][0].round(0) - ext['minx'][0].round(0)
...
# Road masks
os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Roads_mask/resolution_'+str(resolution1)+'m')
...
os.chdir(str(drive)+':/LiDAR_data_processing/'+str(lidar)+'/Snow_depth_processing/'+str(watershed)+'/Snow_free_elevation_masks/'+str(year)+'/resolution_'+str(resolution1)+'m/')
for n in range(len(phases)):
    fn_in = str(extent)+'_'+str(year)+'_'+str(phases[n])+'_SFA.tif'
...
## Merge a bare earth file with a Glacier DEM (applies to Cruickshank)
# Import bare earth
